...
- **pip 缓存清理**：按分类或一键清理全部 pip 缓存，清理前预览大小和文件数
...
- **本地扫描缓存**：扫描结果持久化到本地 JSON，二次启动直接加载，7 天自动过期
- **SSE 实时进度**：扫描进度通过 Server-Sent Events 推送，前端实时渲染，无需轮询
- **Onboarding 引导**：首次使用五步教程，半透明遮罩 + 脉冲边框高亮关键区域，? 按钮可随时重温
...
本批次合并 3 张 ticket，聚焦 UX 增强 — 双向关联、重复环境识别、用户引导。

**新增**
- Python ↔ Venv 双向关联 — Python 表「关联 venv」列显示该 Python 创建的 venv 数量，点击展开子列表并可跳转到 venv 标签页；Venv 表「来源 Python」列可点击，跳转 Python 标签页并高亮闪烁
- 重复环境子标签 — Venv 标签页新增「全部」/「可能重复 (N)」子标签栏；同组重复环境用 8 色调色板彩色左边框标记；空状态提示「未发现重复环境」
- 轻量 Onboarding 引导 — 首次扫描五步教程：统计卡片 → 标签页栏 → 扫描进度 → 表格第一行 → 安全徽章；半透明遮罩 + 脉冲边框高亮；Header「?」按钮可随时重进教程
- 扫描缓存 7 天 TTL — 本地缓存超过 7 天自动过期，等同首次启动

**改进**
- 删除 venv 后自动清理重复组引用，单成员去标记
- 重新进入教程时跳过「扫描中」步骤
