...
格式：`V<主版本>.<次版本>.<修订号>`（语义化版本，SemVer）

| 级别 | 触发条件 | 示例 |
|------|----------|------|
| 主版本（第一位） | 架构重写、不兼容的 API 变更 | `V2.0.0` |
| 次版本（第二位） | 一批完整的 ticket 合并，新增可感知功能 | `V1.1.0` → `V1.2.0` |
| 修订号（第三位） | 单个 hotfix、小修补、文档修正 | `V1.1.0` → `V1.1.1` |

规则：
- 一个次版本 = 一批相关联的 ticket 全部完成并合并
- 单 ticket 的 bugfix 只升修订号
- 在 README 更新日志中记录每次版本号变更
